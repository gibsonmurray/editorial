...
export async function fileToImportedDocument(file: File): Promise<ImportedDocument> {
...
export async function exportDocument(editor: Editor, title: string, format: ExportFormat) {
  const safeTitle = slug(title || 'editorial-document');
  if (format === 'html') {
    downloadBlob(new Blob([editor.getHTML()], { type: 'text/html;charset=utf-8' }), `${safeTitle}.html`);
    return;
  }

  if (format === 'txt') {
    downloadBlob(new Blob([editor.getText({ blockSeparator: '\n\n' })], { type: 'text/plain;charset=utf-8' }), `${safeTitle}.txt`);
    return;
  }
  ...
  if (format === 'md') {
    ...
    downloadBlob(new Blob([turndown.turndown(editor.getHTML())], { type: 'text/markdown;charset=utf-8' }), `${safeTitle}.md`);
    return;
  }
...
function openPrintWindow(html: string, title: string) {
  ...
  if (!win) throw new Error('Could not open a print window. Check popup settings and try again.');
...
async function htmlToDocx(json: JSONContent, title: string) {
  ...
  const children = (json.content ?? []).flatMap(node => nodeToParagraphs(node, docx));
...
function nodeToParagraphs(node: JSONContent, docx: typeof import('docx')) {
  if (node.type === 'heading') {
    return [
      new docx.Paragraph({
        heading: node.attrs?.level === 1 ? docx.HeadingLevel.HEADING_1 : node.attrs?.level === 2 ? docx.HeadingLevel.HEADING_2 : docx.HeadingLevel.HEADING_3,
    ...
    return (node.content ?? []).flatMap((item, index) =>
      (item.content ?? []).map(child =>
        new docx.Paragraph({
          ...
          numbering: node.type === 'orderedList' ? { reference: 'default-numbering', level: 0, instance: index } : undefined,
  ...
  if (node.type === 'blockquote') {
    return (node.content ?? []).map(child => new docx.Paragraph({ children: inlineRuns(child, docx), indent: { left: 360 } }));
  }
  ...
  const visit = (child: JSONContent) => {
    if (child.type === 'text') {
      ...
      for (const mark of child.marks ?? []) {
        ...
        if (mark.type === 'underline') opts.underline = { type: docx.UnderlineType.SINGLE };
      }
...
function slug(value: string) {
  return value.toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'editorial-document';
}
