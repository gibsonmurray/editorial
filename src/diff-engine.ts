...
export const EditOps = {
  computeSegments(originalText: string, edits: EditOp[]): Segment[] {
    ...
    for (const op of edits) {
      ...
      if (matchStart > cursor) {
        const keep: KeepSegment = { id: 'k' + segId++, kind: 'keep', text: originalText.slice(cursor, matchStart) };
        segments.push(keep);
      }
      const edit: EditSegment = { id: 'e' + segId++, kind: 'edit', before, after, status: 'pending' };
      segments.push(edit);
      cursor = matchEnd;
    }

    if (cursor < originalText.length) {
      const keep: KeepSegment = { id: 'k' + segId++, kind: 'keep', text: originalText.slice(cursor) };
      segments.push(keep);
    }
  ...
  pendingCount(segments: Segment[]): number {
    return segments.filter(s => s.kind === 'edit' && s.status === 'pending').length;
  },
