...
export function RichEditor({
  ...
  const editor = useEditor({
    ...
    onUpdate: ({ editor: current }) => {
      onChange(current.getJSON(), current.getHTML(), current.getText({ blockSeparator: '\n\n' }));
    },
    editorProps: {
      attributes: {
  ...
  useEffect(() => {
    if (!editor || !content || !documentId || loadedDocumentId.current === documentId) return;
  ...
  useEffect(() => {
    if (!editor) return;
    updateSuggestionDecorations(editor, suggestions, focusedSuggestionId, inlineDiffs);
