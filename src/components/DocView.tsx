...
export function DocView({ segments, focusedId, setFocusedId, onAccept, onReject, flashIds }: DocViewProps) {
        ...
        return (
          ...
          >
            ...
            <span className="chip-row" contentEditable={false}>
              <button className="accept" title="Accept" onClick={e => { e.stopPropagation(); onAccept(s.id) }} aria-label="Accept">
                <Check size={13} />
              </button>
              <button className="reject" title="Reject" onClick={e => { e.stopPropagation(); onReject(s.id) }} aria-label="Reject">
                <X size={13} />
              </button>
            </span>
          </span>
